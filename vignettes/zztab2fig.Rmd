...
The `zztab2fig` package provides a streamlined workflow to transform R objects
...
- **S3 method dispatch**: `t2f()` now works directly with 20+ object types:
...
- **Inline workflow**: `t2f_inline()` for R Markdown without float positioning,
  with `\captionof` for proper table numbering
- **t2f_regression()**: Side-by-side regression model comparison tables
...
- *Advanced Features* - Footnotes, headers, alignment, placement helpers, and
  inline workflow
- *Why zztab2fig?* - Practical comparison with pander
...
The primary function for converting R objects to LaTeX tables. In v0.2.0, `t2f()`
...
- `theme`: Theme name (`"nejm"`, `"apa"`, `"nature"`, `"minimal"`) or
  `t2f_theme` object.
- `footnote`: A `t2f_footnote` object for table footnotes.
- `header_above`: A `t2f_header_above` object for spanning column headers.
- `collapse_rows`: A `t2f_collapse_rows` object for multi-row cells.
...
```r
t2f_coef(model, caption = "Coefficients")  # defaults: width="3in", align="left"
```

### Traditional Workflow with t2f()
