---
title: "Quickstart Guide: zztab2fig v0.2.1"
...
header-includes:
  ...
  - \usepackage{wrapfig}
vignette: >
  %\VignetteIndexEntry{Quickstart Guide: zztab2fig v0.2.1}
...
This quickstart guide covers the key features in zztab2fig v0.2.1. For complete
...
For reproducible documents, use `t2f()` with `graph_include()`:
```{r graph-include-example, eval=FALSE}
# In an Rmd/qmd code chunk:
t2f(model, theme = "nejm") |> graph_include()
```

The `t2f()` function returns the path to the generated PDF, which
...
2. **Final document**: Use `t2f() |> graph_include()` for reproducible embedding
...
t2f(model, theme = "nejm", caption = "Table 1") |> graph_include()
```
...
t2f_rms_compare(Simple = m1, Full = m2, filename = "rms_comparison")
```

RMS methods: `t2f.ols()`, `t2f.lrm()`, `t2f.cph()`, `t2f.orm()`, `t2f.Glm()`,
`t2f.psm()`, and `t2f_rms_compare()`.


## Extensibility: Any Broom Object

For any object supported by the broom package, use `t2f_tidy()`:

```{r t2f-tidy, eval=FALSE}
library(survival)
...
| `lrm` | `t2f.lrm()` | Logistic regression (OR) |
| `cph` | `t2f.cph()` | Cox proportional hazards (HR) |
| `orm` | `t2f.orm()` | Ordinal regression |
| `Glm` | `t2f.Glm()` | Generalized linear model |
| `psm` | `t2f.psm()` | Parametric survival |
...
Use `t2f_tidy()` for any object type supported by broom.
