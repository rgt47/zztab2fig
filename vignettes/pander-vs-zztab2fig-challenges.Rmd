...
```{r challenge1-pander, eval=FALSE}
df <- mtcars[1:5, 1:4]
...
pdf_path <- file.path(output_dir, "basic_table_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge2-pander, eval=FALSE}
df <- data.frame(
...
pdf_path <- file.path(output_dir, "aligned_table_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge3-pander, eval=FALSE}
df <- iris[1:5, ]
...
pdf_path <- file.path(output_dir, "iris_sample_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge4-pander, eval=FALSE}
...
pdf_path <- file.path(output_dir, "regression_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge5-pander, eval=FALSE}
df <- mtcars[1:5, 1:4]

pander::pander(df, emphasize.strong.rows = 2, emphasize.italics.cols = 3)
```
...
```{r challenge5-display, echo=FALSE, eval=has_latex, out.width="70%"}
pdf_path <- file.path(output_dir, "emphasized_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge6-pander, eval=FALSE}
df <- data.frame(
...
```{r challenge6-display, echo=FALSE, eval=has_latex, out.width="50%"}
pdf_path <- file.path(output_dir, "clinical_footnotes_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge7-pander, eval=FALSE}
df <- data.frame(
...
```{r challenge7-display, echo=FALSE, eval=has_latex, out.width="60%"}
pdf_path <- file.path(output_dir, "spanning_header_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge8-pander, eval=FALSE}
df <- data.frame(
...
```{r challenge8-display, echo=FALSE, eval=has_latex, out.width="55%"}
pdf_path <- file.path(output_dir, "multirow_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge9-pander, eval=FALSE}
df <- data.frame(
  Item = c("A", "B", "C"),
...
```{r challenge9-display, echo=FALSE, eval=has_latex, out.width="45%"}
pdf_path <- file.path(output_dir, "decimal_aligned_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge10-pander, eval=FALSE}
...
df <- mtcars[rep(1:10, 2), 1:4]  # Smaller example for vignette

t2f(df,
    filename = "multipage",
    sub_dir = output_dir,
    caption = "mtcars Data (Repeated)",
    longtable = TRUE)
```

```{r challenge10-display, echo=FALSE, eval=has_latex, out.width="55%"}
pdf_path <- file.path(output_dir, "multipage_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge11-pander, eval=FALSE}
df <- data.frame(
...
```{r challenge11-display, echo=FALSE, eval=has_latex, out.width="55%"}
pdf_path <- file.path(output_dir, "nejm_table_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge12-pander, eval=FALSE}
...
```{r challenge12-zztab2fig, eval=has_latex}
m1 <- lm(mpg ~ cyl, data = mtcars)
m2 <- lm(mpg ~ cyl + hp, data = mtcars)
m3 <- lm(mpg ~ cyl + hp + wt, data = mtcars)
...
```{r challenge12-display, echo=FALSE, eval=has_latex, out.width="70%"}
pdf_path <- file.path(output_dir, "model_comparison_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge13-pander, eval=FALSE}
df <- mtcars[1:5, 1:4]
...
```{r challenge13-zztab2fig, eval=has_latex}
df <- mtcars[1:5, 1:4]

t2f(df, filename = "float_demo", sub_dir = output_dir)

# In LaTeX documents, use:
# t2f_include("float_demo", caption = "Sample Data",
#             label = "fig:sample", position = "htbp")
```

```{r challenge13-display, echo=FALSE, eval=has_latex, out.width="70%"}
pdf_path <- file.path(output_dir, "float_demo_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
...
```{r challenge14-pander, eval=FALSE}
...
```{r challenge14-zztab2fig, eval=has_latex}
df <- data.frame(
...
```{r challenge14-display, echo=FALSE, eval=has_latex, out.width="65%"}
pdf_path <- file.path(output_dir, "complex_table_cropped.pdf")
if (file.exists(pdf_path)) knitr::include_graphics(pdf_path)
```
