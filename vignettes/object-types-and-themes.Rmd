...
zztab2fig v0.2.1 adds full support for Frank Harrell's rms (Regression Modeling
...
t2f(ols_model, output = "anova", filename = "ols_chunk_tests")
...
t2f_rms_compare(Simple = m1, Moderate = m2, Full = m3,
...
zztab2fig supports additional statistical object types through broom
...
zztab2fig method, use `t2f_tidy()`:

```{r t2f-tidy-example, eval=FALSE}
...
t2f_tidy(gam_model, filename = "gam_results")
```

This provides a fallback for the 50+ object types supported by broom that
don't have dedicated zztab2fig methods.
...
| `lrm` | `t2f.lrm()` | `exponentiate`, `output` |
| `cph` | `t2f.cph()` | `exponentiate`, `output` |
| `orm` | `t2f.orm()` | `exponentiate`, `intercepts`, `output` |
| `Glm` | `t2f.Glm()` | `exponentiate`, `output` |
| `psm` | `t2f.psm()` | `output` |
...
| `lme` | nlme | `effects` |
| `Arima` | stats | `digits` |
| Any broom object | `t2f_tidy()` | `tidy_args`, `glance` |
