...
The `zztab2fig` package (v0.2.1) provides a comprehensive solution for
...
package features S3 dispatch for 21+ object types including full rms package
support, journal-specific themes, inline tables for R Markdown, model
comparison tables, and batch processing.
...
| **RMS Package** | ols, lrm, cph, orm, Glm, psm | Coefficient/ANOVA tables |
...
| **Extensibility** | Any broom object | Via t2f_tidy() |
...
t2f(ols_model, filename = "ols_results")

# ANOVA-style chunk tests (useful for splines)
t2f(ols_model, output = "anova", filename = "ols_anova")

# Logistic regression with odds ratios
lrm_model <- lrm(am ~ mpg + wt, data = mtcars)
t2f(lrm_model, exponentiate = TRUE, filename = "logistic_or")

# Cox model with hazard ratios
library(survival)
dd <- datadist(lung)
options(datadist = "dd")
cph_model <- cph(Surv(time, status) ~ age + sex, data = lung)
t2f(cph_model, exponentiate = TRUE, filename = "cox_hr")

# Side-by-side rms model comparison
m1 <- ols(mpg ~ wt, data = mtcars)
m2 <- ols(mpg ~ wt + cyl, data = mtcars)
t2f_rms_compare(Simple = m1, Full = m2, filename = "model_comparison")
```

### Extensibility via t2f_tidy()

For any object supported by broom but without a dedicated method:

```r
# Works with any broom-compatible object
t2f_tidy(any_model, filename = "results")
```
...
| **S3 Dispatch** | 21+ model types | Manual conversion |
...
- RMS package users (biostatistics, clinical trials)
...
```
Thomas, R.G. (2026). zztab2fig: Generate LaTeX Tables and PDF Outputs.
R package version 0.2.1. https://github.com/rgt47/zztab2fig
```
