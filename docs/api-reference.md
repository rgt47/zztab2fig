...
- **Suggests**: broom, broom.mixed, survival, lme4, nlme, rms, tinytex
...
## S3 Methods: RMS Package Models

Require `rms` package. These methods support Frank Harrell's Regression
Modeling Strategies package, commonly used in biostatistics and clinical
trials.

### t2f.ols()

```r
t2f.ols(x, digits = 3, conf.int = TRUE, conf.level = 0.95,
        output = c("coef", "anova"), ...)
```

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `output` | character | "coef" | "coef" for coefficients, "anova" for chunk tests |

### t2f.lrm()

```r
t2f.lrm(x, digits = 3, exponentiate = TRUE, conf.int = TRUE,
        conf.level = 0.95, output = c("coef", "anova"), ...)
```

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `exponentiate` | logical | TRUE | Exponentiate coefficients (odds ratios) |
| `output` | character | "coef" | "coef" for coefficients, "anova" for chunk tests |

### t2f.cph()

```r
t2f.cph(x, digits = 3, exponentiate = TRUE, conf.int = TRUE,
        conf.level = 0.95, output = c("coef", "anova"), ...)
```

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `exponentiate` | logical | TRUE | Exponentiate coefficients (hazard ratios) |
| `output` | character | "coef" | "coef" for coefficients, "anova" for chunk tests |

### t2f.orm()

```r
t2f.orm(x, digits = 3, exponentiate = TRUE, conf.int = TRUE,
        conf.level = 0.95, output = c("coef", "anova"),
        intercepts = FALSE, ...)
```
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `intercepts` | logical | FALSE | Include intercept terms |

### t2f.Glm()

```r
t2f.Glm(x, digits = 3, exponentiate = FALSE, conf.int = TRUE,
        conf.level = 0.95, output = c("coef", "anova"), ...)
```
...
```r
t2f.psm(x, digits = 3, conf.int = TRUE, conf.level = 0.95,
        output = c("coef", "anova"), ...)
```
...
```r
t2f_rms_compare(..., digits = 3, exponentiate = FALSE,
                stars = c(0.05, 0.01, 0.001), filename = "rms_comparison",
                sub_dir = "figures", t2f_args = list())
```

Side-by-side comparison of rms models.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `...` | rms models | required | Named rms model objects |
| `exponentiate` | logical | FALSE | Exponentiate coefficients |
...
t2f_rms_compare(Simple = m1, Full = m2)
```
...
- `t2f_rms_compare()` for side-by-side rms model comparison
- `output = "anova"` parameter for ANOVA-style chunk tests
- Documentation improvements for t2f_tidy() extensibility
- Updated pander comparison with accurate feature counts

### Version 0.2.0 (2024-12)

- S3 generic refactor with methods for 15+ object types
