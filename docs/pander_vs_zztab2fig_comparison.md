# Comparative Analysis: pander vs zztab2fig v0.2.1

A comprehensive comparison of two R packages for table generation and rendering.

**Date:** 2026-02-12 (Updated post-v0.2.1 release with RMS integration)
...
| Dimension | pander | zztab2fig v0.2.1 |
|-----------|--------|------------------|
...
| **Supported Objects** | 50+ R classes | 15 native + broom extensible |
...
| **RMS Package Support** | Yes (cph, lrm, ols) | Yes (full integration) |
...
| R Object Type | pander | zztab2fig v0.2.1 |
|---------------|:------:|:----------------:|
| data.frame | Yes | Yes |
| matrix | Yes | Yes |
| table | Yes | Yes |
...
| survival models (coxph, survreg) | Yes | Yes |
| rms models (ols, lrm, cph, orm) | Yes | Yes (v0.2.1) |
| prcomp/PCA | Yes | Yes |
| kmeans | No | Yes |
| mixed models (lmer, lme) | Partial | Yes |
| ts/zoo time series | Yes | No |
| randomForest | Yes | No (use t2f_tidy) |
| density objects | Yes | No |

**Analysis:** pander maintains broader object coverage through its extensive S3
method library (50+ classes). zztab2fig v0.2.1 now covers 15 object types
natively with focused, publication-ready output. The `t2f_tidy()` function
provides extensibility for any broom-supported object type.
...
### New in zztab2fig v0.2.0 and v0.2.1

| Feature | Description | Version |
|---------|-------------|---------|
| `t2f()` S3 generic | Dispatches to appropriate method based on object class | 0.2.0 |
| `t2f.lm()` | Linear model coefficient tables | 0.2.0 |
| `t2f.glm()` | GLM tables with optional exponentiation | 0.2.0 |
| `t2f.anova()` | ANOVA result tables | 0.2.0 |
| `t2f.htest()` | Hypothesis test result tables | 0.2.0 |
| `t2f.matrix()` | Matrix tables with optional rownames | 0.2.0 |
| `t2f.table()` | Contingency table support | 0.2.0 |
| `t2f.ols()` | RMS ordinary least squares | 0.2.1 |
| `t2f.lrm()` | RMS logistic regression (OR support) | 0.2.1 |
| `t2f.cph()` | RMS Cox proportional hazards (HR support) | 0.2.1 |
| `t2f.orm()` | RMS ordinal regression | 0.2.1 |
| `t2f.Glm()` | RMS generalized linear models | 0.2.1 |
| `t2f.psm()` | RMS parametric survival models | 0.2.1 |
| `t2f_regression()` | Side-by-side model comparison | 0.2.0 |
| `t2f_rms_compare()` | Side-by-side rms model comparison | 0.2.1 |
| `t2f_tidy()` | Any broom-supported object | 0.2.0 |
| `t2f_theme_*()` | Built-in theme constructors | 0.2.0 |
| `t2f_theme_set()` | Global theme management | 0.2.0 |
| `t2f_batch()` | Batch processing API | 0.2.0 |
| `t2f_format()` | Cell-level formatting | 0.2.0 |
| `t2f_highlight()` | Conditional highlighting | 0.2.0 |
| `t2f_cache_*()` | Caching utilities | 0.2.0 |
| Custom knitr engine | Native R Markdown integration | 0.2.0 |
| `output = "anova"` | ANOVA-style chunk tests for rms | 0.2.1 |
...
| Category | pander | zztab2fig v0.2.1 | Winner |
|----------|:------:|:----------------:|--------|
| Output format variety | 5 formats | 4 formats | pander |
| Object type coverage | 50+ classes | 15 native + broom | pander |
...
With version 0.2.1, zztab2fig has evolved from a simple data-frame-to-PDF
converter into a comprehensive LaTeX table generation system with particular
strength in biostatistics workflows. While pander maintains advantages in
output format variety and object type coverage, zztab2fig now offers superior
capabilities for academic publishing workflows:

- Native handling of statistical objects (lm, glm, anova, htest)
- Full RMS package integration (ols, lrm, cph, orm, Glm, psm)
...
- Extensibility via `t2f_tidy()` for any broom-supported object
...
LaTeX tables with journal-specific styling. For clinical trials and
biostatistics work using Frank Harrell's rms package, zztab2fig now provides
