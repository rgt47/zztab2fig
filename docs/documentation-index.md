...
package (v0.2.1), covering technical specifications, architectural design,
...
- S3 generic dispatch for 21+ object types (lm, glm, coxph, rms, etc.)
- Full rms package support for biostatistics workflows (v0.2.1)
- Five built-in journal themes (minimal, apa, nature, nejm, lancet)
- Inline tables for R Markdown with frame/background styling
- Model comparison tables via `t2f_regression()` and `t2f_rms_compare()`
...
| RMS | t2f.ols, t2f.lrm, t2f.cph, t2f.orm, t2f_rms_compare |
| Extensibility | t2f_tidy |
...
| S3 method documentation | All 21+ methods | Complete |
| RMS method documentation | All 6 methods | Complete |
...
package v0.2.1 from multiple perspectives: user guidance, API reference,
technical specifications, and architectural design. Version 0.2.1 adds full
rms package support for biostatistics workflows and extensibility via
`t2f_tidy()`. The structured approach supports users at all technical levels
while maintaining scholarly standards appropriate for academic and
