...
The `zztab2fig` package (v0.2.1) converts R data frames and statistical
objects into publication-quality LaTeX tables with automatic PDF generation.
The package supports S3 dispatch for common model objects including full rms
package support, journal-specific themes, inline tables for R Markdown, and
...
| Class | Method | Key Options |
|-------|--------|-------------|
| `ols` | `t2f.ols()` | `output` (coef/anova) |
| `lrm` | `t2f.lrm()` | `exponentiate`, `output` |
| `cph` | `t2f.cph()` | `exponentiate`, `output` |
| `orm` | `t2f.orm()` | `exponentiate`, `intercepts` |
| `Glm` | `t2f.Glm()` | `exponentiate`, `output` |
| `psm` | `t2f.psm()` | `output` |
...
|----------|-------------|
| `t2f_tidy()` | Any broom-supported object |
| `t2f_rms_compare()` | Side-by-side rms model comparison |
