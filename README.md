...
|:------------|:-------|:------------|
| `ols` | `t2f.ols()` | Ordinary least squares |
| `lrm` | `t2f.lrm()` | Logistic regression (OR support) |
| `cph` | `t2f.cph()` | Cox proportional hazards (HR support) |
| `orm` | `t2f.orm()` | Ordinal regression |
| `Glm` | `t2f.Glm()` | Generalized linear models |
| `psm` | `t2f.psm()` | Parametric survival models |

Additional types via broom integration: `coxph`, `survreg`, `nls`, `polr`,
`prcomp`, `kmeans`, `lmerMod`, `glmerMod`, `lme`, `Arima`.

**Any broom-supported object** works via `t2f_tidy()`:

```r
t2f_tidy(any_broom_object, filename = "result")
```
...
| `t2f_rms_compare()` | Compare rms models |
